...
public class Board {
	...
	private final boolean AM_DEBUGGING = false;
	...
	private boolean isValidWord(Tile[][] oldBoard, Tile[][] newBoard) {
		...
		if (isHorizontal(oldBoard, newBoard)) {
			int row = newIndexes(oldBoard, newBoard)[0];
			for (int col = 0; col < 15; col++) {
				if (word.size() > 0 && newBoard[row][col].isEmpty()) {
					break;
				} else if (!newBoard[row][col].isEmpty()) {
					word.add(newBoard[row][col].getLetter());
				}
			}
		} else {
			int col = newIndexes(oldBoard, newBoard)[1];
			for (int row = 0; row < 15; row++) {
				if (word.size() > 0 && newBoard[row][col].isEmpty()) {
					break;
				} else if (!newBoard[row][col].isEmpty()) {
	...
	private int[] newIndexes(Tile[][] oldBoard, Tile[][] newBoard) {
		int rRow = -1, rCol = -1;
